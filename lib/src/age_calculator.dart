...
class AdvancedAgeCalculator {
  ...
  static AgeDuration calculateAge({
    ...
    DateTime convertedBirthDate = CalendarHelper.convertToGregorian(birthDate, calendarSystem);
    DateTime convertedCurrentDate = CalendarHelper.convertToGregorian(currentDate, calendarSystem);
    ...
    if (timezone != null) {
      ...
      convertedBirthDate = TimezoneHelper.convertToTimezone(convertedBirthDate, timezone);
      convertedCurrentDate = TimezoneHelper.convertToTimezone(convertedCurrentDate, timezone);
    }
    ...
    // Calculate totals
    Duration totalDuration = convertedCurrentDate.difference(convertedBirthDate);
  ...
  static AgeDuration calculateAgeDifference({
    ...
    DateTime olderDate = birthDate1.isBefore(birthDate2) ? birthDate1 : birthDate2;
    DateTime youngerDate = birthDate1.isBefore(birthDate2) ? birthDate2 : birthDate1;
  ...
  static Map<String, DateTime> calculateLifeMilestones({
    ...
    return {
      'quarter_life': birthDate.add(Duration(days: (lifeExpectancy * 365.25 / 4).round())),
      'half_life': birthDate.add(Duration(days: (lifeExpectancy * 365.25 / 2).round())),
      'three_quarter_life': birthDate.add(Duration(days: (lifeExpectancy * 365.25 * 0.75).round())),
      'estimated_end': birthDate.add(Duration(days: (lifeExpectancy * 365.25).round())),
    };
  }
  ...
  static double calculateLifeProgress({
    ...
    currentDate ??= DateTime.now();
    AgeDuration age = calculateAge(birthDate: birthDate, currentDate: currentDate);
